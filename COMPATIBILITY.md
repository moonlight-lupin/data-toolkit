...
|---|---|---|---|
| data-tidy | any (portable); local processing | Python + `openpyxl` | `PyMuPDF` for PDF input, `python-docx` for .docx, `extract_msg` for .msg (each optional, degrade per-source); **scanned-PDF OCR needs local Tesseract** (digital PDFs/spreadsheets unaffected) |
| data-extract | any (portable); local processing | Python + `PyMuPDF`, `openpyxl` | `python-docx` for .docx, `extract_msg` for .msg (optional); **scanned-document OCR needs local Tesseract**; shares the engine at `scripts/` (`dataclean.py`/`ingest.py`/`extract.py`) |
